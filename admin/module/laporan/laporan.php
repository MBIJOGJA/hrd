...
<form class="form-horizontal" action="module/laporan/habis_kontrak.php" method="post">             
  ...
  <input type="date" name="tanggal" required="required" class="form-control pull-right" id="reservation"/>
</div><!-- /.input group -->
</div>
<div class="col-sm-1">
...
<?php 
// Tampilkan data dari Database
$sql = "SELECT * FROM sk_krj sk JOIN pegawai p ON sk.nip=p.nip JOIN jabatan j ON j.id_jabatan=sk.id_jabatan JOIN unit_krj u ON u.id_unit_krj=sk.id_unit_krj WHERE sk.status_sk='aktif'";
$tampil = mysql_query($sql);
$no=1;
while ($data = mysql_fetch_array($tampil)) { ?>
    ...
    <td><?php echo $data['nip']; ?></td>
    <td><?php echo $data['nm_pegawai']; ?></td>
    <td><?php echo $data['tgl_sk'] .", ". IndonesiaTgl($data['tgl_lhr']); ?></td>
