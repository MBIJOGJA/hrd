...
<i class="fa fa-user-md"></i> Informasi Pegawai </h3>
  ...
  <i class="fa fa-minus"></i></a>
    ...
    <?php
  $hasil1 = mysql_query("SELECT max(SUBSTRING(nip,7,10)) as terakhir from pegawai"); $data = mysql_fetch_array($hasil1);
  $lastID1 = $data['terakhir']; $nextNoUrut1 = $lastID1 + 1;
  $nextID1 = "".date('Ym')."".sprintf("%04s",$nextNoUrut1);
  ?>
  <div class="box-body">
  <div class="form-group">
    <label class="col-sm-4 control-label">NIP</label>
    <div class="col-sm-5">
      <input type="text" class="form-control" required="required" name="nip" value="<?php echo $nextID1; ?>" >
    </div>
  </div>
  <div class="form-group">
...
<div id="main">
   ...
   <center><a class="ad-box btn btn-primary"" href="#"> <i class="fa fa-plus"> </i> Tambah</a></center> 
</div>
<script type="text/javascript">
  $(document).ready(function() {
...
<?php $q = mysql_query ("select * from status_pegawai");
while ($k = mysql_fetch_array($q)){ ?>
<option value="<?php echo $k['id_status_pegawai']; ?>" <?php if(($k['id_status_pegawai'])== ($data['id_status_pegawai']))
        {echo "selected=\"selected\"";};?>
<?php (@$h['status_pegawai']==$k['status_pegawai'])?print(" "):print(""); ?>  > <?php echo $k['id_status_pegawai']; echo " / ".$k['status_pegawai']; ?>
</option> <?php } ?>
</select>
...
<?php $q = mysql_query ("select * from jjg");
while ($k = mysql_fetch_array($q)){ ?>
<option value="<?php echo $k['id']; ?>" <?php if(($k['id'])== ($data['id']))
        {echo "selected=\"selected\"";};?>
<?php (@$h['jjg']==$k['jjg'])?print(" "):print(""); ?>  > <?php echo $k['id']; echo " / ".$k['nm_jjg']; ?>
</option> <?php } ?>
</select>
  </div><div class="col-sm-3">
