...
//Tambah
	...
	$sq="INSERT INTO sk_krj  (no_sk,nip, tgl_sk, tgl_selesai, id_jabatan, id_unit_krj,id_status_pegawai, status_sk,jenis_sk) VALUES ( '$no_sk','$nip','$tgl_msk','$tgl_selesai','$id_jabatan','$id_unit_krj','$id_status_pegawai','$status','SK')";
...
//input data pendidikan
	for ($i = 0; $i < $num_input; $i++) {
		...
		$jjg = $_POST['jjg'];
		$nm_jjg = $_POST['nm_jjg'];
		$thn_jjg = $_POST['thn_jjg'];
		$query ="insert into pendidikan (nip, thn_pend, jenjang, nm_pendidikan) values('$nip','$thn_jjg[$i]', '$jjg[$i]','$nm_jjg[$i]')";
		$simp = mysql_query($query);
		echo $query;
	}
//input riwayat kerja
 ...
 if($simpan and $simpa and $simp)
 { 
 echo " <script>alert('Input Data Karyawan Berhasil');</script> ";
 echo " <script>window.location='../../index.php?module=pegawai'; </script>";
}
 	else{
 		echo " <script>alert('Input Data Karyawan Berhasil');</script> ";
 		echo " <script>window.location='../../index.php?module=pegawai'; </script>";
